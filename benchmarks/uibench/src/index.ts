...
const Table = component(() => (state: TableState) => (
  htm`
    table.Table
      tbody
...
const Anim = component(() => (state: AnimState) => (
  htm`div.Anim ${List(state.items, animId, AnimBox)}`
));

const TreeLeaf = component(() => (state: TreeNodeState) => (
  htm`li.TreeLeaf =${state.id}`
));
...
const TreeNode = component(() => (state: TreeNodeState) => (
  htm`ul.TreeNode ${List(state.children, treeNodeId, subtree)}`
));

const Tree = component(() => (root: TreeNodeState) => (
  htm`div.Tree ${TreeNode(root)}`
));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  uibench.run(
    (next) => { updateRoot(root, Main(next)); },
    (samples) => { container.innerHTML = `<pre>${JSON.stringify(samples, void 0, 2)}</pre>`; },
  );
});
